...
### React如何实现组件通信
父子：props
子父：父组件将函数作为props传递给子组件，子组件将需要传递的参数放入来自父组件的回调函数中
...
### diff算法是怎么运作
**Diff算法是一种对比算法**，对比两者都是新旧虚拟DOM，对比哪个虚拟Dom做出了更改，并更新这个虚拟节点对应的真实节点，不用更新其他数据没有发生改变的节点，提高效率。

### React Fiber 具体做了哪些工作
Fiber：react内部实例树的实现以及负责操作树的算法
### React如何实现时间切片
react中有许多微小的任务，即所有工作中的fiber节点，执行调度工作循环、计算工作循环，执行每一个工作中的fiber，但是每5毫秒，跳出工作循环，运行异步Message Channel(浏览器提供的一种数据通信接口)的port.postMessage(),检查是否存在事件响应、有更高优先级任务执行，如果有则执行，没有则重新创建循环，执行剩下的fiber
#### 为什么vue不用时间切片
...
```
- 注意 
外层为React.PureComponent，render传递的组件为React.Component，因浅比较props总会得到false，内部用一个函数方法去接Component render的内容，再传给PureComponent
外层
- 缺陷
...
#### PureComponent 与 React.memo 与shouldComponentUpdate
React.memo 类比PureComponent,是高阶组件，函数组件类组件都能用，只对props的情况确定是否渲染
PureComponent针对props、state

React.memo: 第二个参数 返回 true 组件不渲染 ， 返回 false 组件重新渲染。 shouldComponentUpdate: 返回 true 组件渲染 ， 返回 false 组件不渲染。
### props、state区别
props外部传入；state内部自行管理，constructor中定义
